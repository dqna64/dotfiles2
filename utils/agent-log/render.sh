...
#
# Usage: render.sh [dir] [-n <count>] [-a]
#   dir     project to read (default: the git root above $PWD, else $PWD)
#   -n      show only the most recent <count> entries
#   -a      every project's log, merged - what have agents been doing lately
#
# The log is stored as JSONL because many processes append to it concurrently
# and only a one-line-per-write format is safe to share. That's a write-side
# constraint, not a reading experience, so the human view is generated here.
#
...
if [ "$all" = true ]; then
	logs=( "$(agent_logs_dir)"/*.jsonl )
	[ -e "${logs[0]:-}" ] || { printf 'No agent logs in %s\n' "$(agent_logs_dir)" >&2; exit 1; }
	title="All projects"
	subtitle="$(agent_logs_dir)"
else
	[ -n "$dir" ] || dir="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
	logs=( "$(agent_log_path "$dir")" )
	[ -s "${logs[0]}" ] || { printf 'No agent log for %s\n  (expected %s)\n' "$dir" "${logs[0]}" >&2; exit 1; }
	title="$(basename "$dir")"
	subtitle="$dir"
fi
